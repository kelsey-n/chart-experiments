...
export type RawHierarchy = { request_hierarchy: RawNode[] };
...
/** One-level projection using per-level values, filtered and **sorted desc** by value. */
function projectLevel(
  ...
  const kids = collectAtLevel(subtree, level).map((rn) => {
  ...
  const filteredSorted = kids.filter(
    (k) => Number.isFinite(k.value) && (k.value as number) > 0
  );
  // .sort((a, b) => (b.value ?? 0) - (a.value ?? 0)); // DESC

  return { name: `level-${level}`, level, children: filteredSorted };
}
...
type TreemapMVPProps = {
  data: RawHierarchy;
  metric: MetricMode;
  valueSource?: ValueSource; // "count" | "pct"
  geoCode?: string;
  initialLevel?: 2 | 1 | 0; // default 2 (top)
  paddingInner?: number;
  paddingOuter?: number;
  minLabelPx?: number;
  className?: string;
...
export default function TreemapMVP({
  ...
  // Focus is now a **stack** of RawNodes from syntheticRoot -> ... -> current
  const syntheticRoot = useMemo(() => makeSyntheticRoot(data), [data]);
  const [stack, setStack] = useState<RawNode[]>([syntheticRoot]);

  // derive current focus and level from the stack
  const focusRaw = stack[stack.length - 1];
  const level = initialLevel - (stack.length - 1); // e.g., start at 2; after one drill => 1

  // reset stack if data/initialLevel changes
  useEffect(() => {
    setStack([syntheticRoot]);
  }, [syntheticRoot]);

  // Build the per-level projection for the current focus+level
  ...
  const root = useMemo(() => {
    const h = d3
      ...
      .sum((d) => d.value ?? 0)
      .sort((a, b) => (b.value ?? 0) - (a.value ?? 0)); // <-- canonical sort

    d3
      .treemap<TreeNode>()
      .tile(d3.treemapBinary) // still fine; sort mainly affects ties / other tilers
  ...
  // drill if raw children include nodes at NEXT LOWER level
  ...
  // go up: pop the focus stack if possible
  ...
  // Breadcrumb: path names (exclude synthetic root)
  const crumb = useMemo(() => {
    const names = stack.slice(1).map((n) => n.cluster_name);
    const path = names.length ? names.join(" › ") : "All";
    const metricStr = `${metric}${
      metric !== "global" && geoCode ? `:${geoCode}` : ""
    }`;
    return `${path} • Level ${level} • ${metricStr} • ${valueSource}`;
  }, [stack, level, metric, geoCode, valueSource]);

  return (
    <div ref={wrapRef} className={`treemap-wrap ${className ?? ""}`}>
      <svg className="treemap-svg" role="img" aria-label="Treemap">
        {/* No need for background click anymore; use the Back button */}
        <rect
            ...
            <g
              ...
              onClick={(e) => {
                ...
                if (drillable && tn.__raw) {
                  setStack((s) => [...s, tn.__raw]); // push child; level derives from length
                }
              }}
              onKeyDown={(e) => {
                if (
      ...
      <div className="treemap-help">
        Enter/Click to drill (2→1→0) • Esc/Back to go up
      </div>
    </div>
  );
}
